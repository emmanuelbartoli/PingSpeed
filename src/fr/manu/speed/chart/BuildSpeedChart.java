 ...
 *
 */
public class BuildSpeedChart {

	/**
	 * 
	 */
	private static LogService log = new LogService(
			BuildSpeedChart.class.getName(),
			BuildSpeedChart.class.getSimpleName());
	 ...
	 * 
	 */
	private String MAIN_HTML="fr/manu/speed/chart/speed.html";
	
	/**
	 * 
	 */
	private String SPEED_JS="fr/manu/speed/chart/speed.js";
	
	/**
	 * 
	 */
	private String BODY_HTML="fr/manu/speed/chart/body.html";
	 ...
	 * @param confFilePath
	 */
	public BuildSpeedChart(String confFilePath) {
	...
	public void execute() {
		...
		try {
			...
			// Speed html chart file
			String speedFilePath  = resource.getString("speed.html.chart.file");
			...
			for (Interval interval : Interval.values()) {
				...
				//////////////////////////////////////////
				// Read the speed list from the database //
				//////////////////////////////////////////
				speeds 	  = SpeedDAO.getSpeeds(connection.getConnection(),interval.getSql());
				downUp = new StringBuffer();
				...
				// Javascript Code for the speed values
				for (Speed speed:speeds) {
					// The min/max/average values
					downUp
					.append("[")
					.append("'").append(speed.getDate()).append("'").append(",")
					.append(speed.getDownload()).append(",")
					.append(speed.getUpload()).append(",")
					.append("]").append(",");
					
				}
				downUpHtml = downUp.toString();
	
		        // The javascript template
		        javascript = getTemplate(SPEED_JS);
		        ...
		        javascript = javascript
		        		.replace("#TYPE#"		    , interval.name())
		     			.replace("#DOWNUP#"		    , downUpHtml)
		     			.replace("#SPEED#"          ," ("+interval.getLabel()+")");
	        ...
	        ////////////////////////////////
	     	// Write the output speed file //
	        ////////////////////////////////
	        File speedFile=new File(speedFilePath);
	     	FileUtils.writeStringToFile(speedFile, mainHtml,"UTF-8");
	     	
	     	//log
	     	log.info("The file "+speedFilePath+" has been created successfully.");
	     	...
	     	if (isReachable(ftpServer,21,3000)) {
		        ...
		        client.upload(speedFile);
		        client.disconnect(true);
		        log.info("The file "+speedFile.getName()+" has been uploaded successfully to "+ftpUser+"@"+ftpServer+".");
	     	}
	     	else {
	...
	public static void main(String[] args) {
		...
		// Run the daemon
		new BuildSpeedChart(confFilePath).execute();
	}
